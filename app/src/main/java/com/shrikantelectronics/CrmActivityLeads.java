...
public class CrmActivityLeads extends AppCompatActivity {
	...
	public void invokeWS_DailySales() {
		...
		try {
			...
			header_salesregister.setText("WALKIN FOLLOWUP -" + employeename);

			ApiHelper.post(URL + "Service1.asmx/CRM_DailyLeads_Employee", paramsMap, new ApiHelper.ApiCallback()   {
				@Override
				...
				public void onSuccess(JSONObject response) {
					try {
						...
						TableRow tblrowHeading = new TableRow(CrmActivityLeads.this);
						...
						for (int i = 0, count = new_array.length(); i < count; i++) {
							try {
								...
								TableRow tblrowLabels = new TableRow(CrmActivityLeads.this);
								...
								highsLabel_custname.setOnClickListener(new View.OnClickListener()
								{
									...
									{
										Intent intent = new Intent(CrmActivityLeads.this, Salesmen_Followup_Customer_Single.class);
										intent.putExtra("walkin_custcd",custcd);
										intent.putExtra("syscustactno",syscustactno);
										intent.putExtra("customer_custcd","0");
	...
	private TextView initPlainTextView(float n) {
		...
		TextView textView = new TextView(CrmActivityLeads.this);
