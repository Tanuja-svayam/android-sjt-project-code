...
public class StockVerificationTransporter_view extends AppCompatActivity {
    ...
    ListViewAdapter_StockVerification_Transporter adapter;
    private ListView lv;
    EditText inputSearch;
    ArrayList<HashMap<String, String>> Transporter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        invokeWS_Transporter();
    }

    public void invokeWS_Transporter(){
        try {

            Map<String, String> paramsMap = new HashMap<>();
            paramsMap.put("transportercd", "");
            paramsMap.put("transportername", "");

            //AsyncHttpClient client = new AsyncHttpClient();
            ApiHelper.post(URL + "Service1.asmx/GetTransporterList", paramsMap, new ApiHelper.ApiCallback()   {
            @Override

            public void onSuccess(JSONObject response) {
                try {
                    JSONObject obj = response;
                    JSONArray new_array = obj.getJSONArray("Transporter");
                    for (int i = 0, count = new_array.length(); i < count; i++) {
                        try {
                            JSONObject jsonObject = new_array.getJSONObject(i);
                            Transporter wp = new Transporter(jsonObject.getString("transportercd"),jsonObject.getString("name"));
                            arraylist.add(wp);
                    ...
                    adapter = new ListViewAdapter_StockVerification_Transporter(StockVerificationTransporter_view.this, arraylist);
            ...
            case RESULT_SPEECH: {
                if (resultCode == RESULT_OK && null != data) {
                    ...
                    else
                    {
                        ...
                        Toast.makeText(StockVerificationTransporter_view.this,text.get(0), Toast.LENGTH_LONG).show();
                        ...
                        else if(text.get(0).equals("what do you think of Kaylee"))
                        {
                           ...
                           // popUpSearchResult("Book_table", "TITLE", text.get(0).toUpperCase());
                            ...
                            //Make this generic by reading whichever books transporternames
